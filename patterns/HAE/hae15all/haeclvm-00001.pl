...
# Description: Confirm locking_type is 1 if lvmlockd resources found
# Modified:    2022 Jan 11
...
@PATTERN_RESULTS = (
	...
	PROPERTY_NAME_CATEGORY."=lvmlockd",
	PROPERTY_NAME_COMPONENT."=Locking",
	PROPERTY_NAME_PATTERN_ID."=$PATTERN_ID",
	PROPERTY_NAME_PRIMARY_LINK."=META_LINK_TID",
	PROPERTY_NAME_OVERALL."=$GSTATUS",
	PROPERTY_NAME_OVERALL_INFO."=None",
	"META_LINK_TID=http://www.suse.com/support/kb/doc.php?id=xxxxxxx"
);
...
sub lvmResourcesFound {
	...
	foreach $_ (@CONTENT) {
		next if ( m/^\s*$/ ); # Skip blank lines
		if ( /<primitive.*provider="lvmlockd/i ) {
			SDP::Core::printDebug("PROCESSING", $_);
			$RCODE++;
			last;
	}
	...
	if ( SDP::SUSE::haeEnabled() ) {
		if ( lvmResourcesFound() ) {
			if ( invalidLockingType() ) {
				SDP::Core::updateStatus(STATUS_CRITICAL, "Invalid LVM locking type for lvmlockd. Remove or set locking_type to 1 in lvm.conf");
			} else {
